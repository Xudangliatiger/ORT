...
c = OmegaConf.load(Path(__file__).resolve().parents[1] / 'configs/ort_alitok_xl.yaml')
...
Dataset.from_dict({'label': [i % 10 for i in range(32)], 'tokens': [[i % 32] * 273 for i in range(32)]}).save_to_disk(str(r / 'data'))
